...
const Recommendations = ({navigation, keywords}) => {
    ...
    return (
      <View style={{marginRight: 5}}>
        <ProductCardGrid
          productInfo={item}
          onPress={() =>
            navigation.navigate('product_details_recommendations', {
              item: item,
              disableRecommendations: true,
            })
          }
          containerStyle={productCardContainer}
          imageContainerStyle={productImageContainer}
        />
      </View>
    );
  };
  ...
  const RenderHeader = () => {
    return <Text style={headerTextStyle}>Similar Products</Text>;
  };
